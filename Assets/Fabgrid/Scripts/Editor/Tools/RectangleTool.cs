    ...
    public class RectangleTool : Tool
    {
        ...
        private Vector2 GetSpacing()
        {
            ...
            else if (tilemap.rectangleToolOffsetOption == OffsetOption.Relative)
            {
                ...
                {
                    var bounds = FabgridUtility.GetTileWorldBoundsWithRotation(tilemap.selectedTile.prefab, tilemap.selectedTile.sizeCalculationOption, tilemap.selectedTile, tilemap);
                    return new Vector2(bounds.size.x * tilemap.rectangleOffsetMultiplier.x, bounds.size.z * tilemap.rectangleOffsetMultiplier.y);
                }
            }
