...
namespace Fabgrid
{
    ...
    public class Tile
    {
        ...
        public Bounds GetWorldBounds(Vector3 position, Tilemap3D tilemap)
        {
            if (position.IsInfinity() || prefab == null)
                return new Bounds(Vector3.zero, Vector3.one);

            return new Bounds(offset + position, size);
        }

        public Vector3 GetCenterToSurfaceVector(Vector3 position, Vector3 direction, Tilemap3D tilemap)
        {
            if (position.IsInfinity()) return Vector3.negativeInfinity;

            var worldBounds = GetWorldBounds(position, tilemap);
        ...
        public Vector3 GetOffset(Vector3 position, Tilemap3D tilemap)
        {
            var center = GetWorldBounds(position, tilemap).center;
