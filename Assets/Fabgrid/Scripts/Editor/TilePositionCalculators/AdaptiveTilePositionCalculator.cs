        ...
        public override Vector3 GetPosition(Vector2 mousePosition)
        {
            ...
            if (tilemap.selectedGameObject != null && tilemap.selectedGameObject.transform.position.y >= floorPosition.y)
            {
                ...
                if (hit.transform != null)
                {
                    ...
                    tilePosition += tilemap.selectedTile.GetOffset(tilePosition, tilemap);
                    tilePosition += Vector3.up * tilemap.selectedTile.GetWorldBounds(tilePosition, tilemap).extents.y;
            ...
            {
                tilePosition += tilemap.selectedTile.GetCenterToSurfaceVector(tilePosition, Vector3.down, tilemap);
                tilePosition += tilemap.selectedTile.GetOffset(tilePosition, tilemap);
            }
