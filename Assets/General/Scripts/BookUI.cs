...
namespace UnityEngine.UI.Extensions
{
    ...
    public class BookUI : MonoBehaviour
    {
        ...
        Material material
        {
            get
            {
                if (_mat == null)
                {
                    ...
                    var matrix = Matrix4x4.Scale(new Vector3(1f / Resolution.x, 1f / Resolution.y, 1f))
                                 * CalcCanvas2LocalMatrix();
        ...
        void Awake()
        {
            // Ищем ID для параметра _Page в шейдере
            PageID = Shader.PropertyToID("_Page");

            // Присваиваем материал со шейдером "PageDeform" 
            // только обычным UI-графическим элементам,
            // НО пропускаем TextMeshProUGUI (чтобы не ломать SDF)
            foreach (var g in GetComponentsInChildren<Graphic>(true))
            {
                // Проверяем, что это НЕ TextMeshProUGUI
                if (g is TextMeshProUGUI)
                {
                    // Если вы хотите искажать и текст,
                    // нужно править сам шейдер TMP, 
                    // а не подменять его на "UI/PageDeform".
                    continue;
                }
        ...
        Matrix4x4 CalcCanvas2LocalMatrix()
        {
            ...
            return transform.worldToLocalMatrix
                   * canvaslist[canvaslist.Length - 1].transform.worldToLocalMatrix.inverse;
        }
